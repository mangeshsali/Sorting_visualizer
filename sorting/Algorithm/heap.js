...
function max_heapify(n, i) {
  ...
  if (l < n && div_sizes[l] > div_sizes[largest]) {
    if (largest != i) {
      div_update(divs[largest], div_sizes[largest], "rgb(59 130 246);"); //Color update
    }

    largest = l;

    div_update(divs[largest], div_sizes[largest], "rgb(239 68 68);"); //Color update
  }
...
function heap_sort() {
  ...
  for (var i = array_size - 1; i > 0; i--) {
    swap(0, i);
    div_update(divs[i], div_sizes[i], "rgb(34 197 94);"); //Color update
    div_update(divs[i], div_sizes[i], "rgb(234 179 8);"); //Color update
