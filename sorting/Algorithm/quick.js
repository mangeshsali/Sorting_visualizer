...
function quick_partition(start, end) {
  ...
  div_update(divs[start], div_sizes[start], "rgb(234 179 8);"); //Color update
  ...
  for (var j = start + 1; j <= end; j++) {
    ...
    if (div_sizes[j] < piv) {
      ...
      div_update(divs[i], div_sizes[i], "rgb(59 130 246);"); //Height update
      div_update(divs[j], div_sizes[j], "rgb(59 130 246);"); //Height update

      i += 1;
    }
  }
  div_update(divs[start], div_sizes[start], "rgb(239 68 68);"); //Color update
  ...
  div_update(divs[start], div_sizes[start], "rgb(239 68 68);"); //Height update
  div_update(divs[i - 1], div_sizes[i - 1], "rgb(239 68 68);"); //Height update

  for (var t = start; t <= i; t++) {
    div_update(divs[t], div_sizes[t], "rgb(34 197 94);", div_sizes[t]); //Color update
  }
